...
int main(void)
{
  ...
  /* creation of Task01 */
  Task01Handle = osThreadNew(Task1Funkc, NULL, &Task01_attributes);
  ...
  /* USER CODE BEGIN 5 */
	char pressed = 1;
	    ...
	    for(;;)
	    {
	        ...
	        {
	            osMessageQueuePut(myQueue01Handle, &pressed, 0, 0);
...
/* USER CODE BEGIN Header_Task1Funkc */
/**
* @brief Function implementing the Task01 thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_Task1Funkc */
void Task1Funkc(void *argument)
{
  /* USER CODE BEGIN Task1Funkc */
	uint32_t zacetek;
	uint32_t reakcija;
	char dummy;
	...
	for(;;)
	{
		...
		uint32_t rd;
		HAL_RNG_GenerateRandomNumber(&hrng, &rd);

		//osDelay(rd);
		osDelay((HAL_GetTick()%4000) + 1000);

		HAL_GPIO_WritePin(LED_RED_GPIO_Port, LED_RED_Pin, GPIO_PIN_RESET);

		zacetek = HAL_GetTick();

		osMessageQueueGet(myQueue01Handle, &dummy, NULL, osWaitForever);

		reakcija = HAL_GetTick() - zacetek;

		HAL_GPIO_WritePin(LED_RED_GPIO_Port, LED_RED_Pin, GPIO_PIN_SET);

		osMessageQueuePut(myQueue02Handle, &reakcija, 0, 0);

		//osDelay(50);
	}
  /* USER CODE END Task1Funkc */
}
